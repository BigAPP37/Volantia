...
export function Header({ title = 'Volantia', showProfile = true }: HeaderProps) {
  ...
  // Reactive notifications list — updated on every action
  ...
  } = useNotifications(entries, settings);

  // Sync notifications into state whenever panel opens or actions happen
  ...
  // Refresh on open and on interval
  useEffect(() => {
    refreshNotifs();
  ...
  // Wrapped actions that immediately refresh state
  const handleMarkRead = useCallback((id: string) => {
    markRead(id);
    refreshNotifs();
  }, [markRead, refreshNotifs]);

  const handleMarkAllRead = useCallback(() => {
    markAllRead();
    refreshNotifs();
  }, [markAllRead, refreshNotifs]);

  const handleClearAll = useCallback(() => {
    clearAll();
    refreshNotifs();
  }, [clearAll, refreshNotifs]);
  ...
  const handleBellClick = () => {
    if (!notifOpen) {
      // Mark all read immediately when opening
      markAllRead();
      refreshNotifs();
    }
    setNotifOpen(v => !v);
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="relative flex h-9 w-9 items-center justify-center rounded-xl transition-transform active:scale-95"
...
export function Header({ title = 'Volantia', showProfile = true }: HeaderProps) {
  ...
  const {
    requestPermission, getPermission, getNotifications,
    getUnreadCount, markAllRead, markRead, clearAll,
  } = useNotifications(entries, settings);
